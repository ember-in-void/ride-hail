...
func NewHTTPServer(svc service.ServiceInterface, port int, logger *logger.Logger) *Server {
...
func newRouter(svc service.ServiceInterface, logger *logger.Logger) *http.ServeMux {
	router := http.NewServeMux()
	Routes(router, svc, logger)
	return router
}

func NewHandler(svc service.ServiceInterface, logger *logger.Logger) *Handler {
	return &Handler{
		svc:    svc,
		logger: logger,
	}
...
func Routes(router *http.ServeMux, svc service.ServiceInterface, logger *logger.Logger) {
...
func (h *Handler) CreateRide(w http.ResponseWriter, r *http.Request) {
	...
	var req ride_models.RideRequest
